...
import io.trino.plugin.pinot.auth.basic.PinotBasicAuthenticationProvider;
import io.trino.plugin.pinot.auth.basic.PinotBrokerBasicAuthenticationConfig;
import io.trino.plugin.pinot.auth.basic.PinotControllerBasicAuthenticationConfig;
        ...
        extends AbstractConfigurationAwareModule
{
    ...
    protected void setup(Binder binder)
    {
        ...
        bindAuthenticationProviderModule(
                BASIC_INLINE,
                new PinotBasicControllerAuthenticationProviderModule(),
                new PinotBasicBrokerAuthenticationProviderModule());
    }
    ...
    private static class PinotBasicControllerAuthenticationProviderModule
            implements Module
    {
        @Override
        public void configure(Binder binder)
        {
            configBinder(binder).bindConfig(PinotControllerBasicAuthenticationConfig.class);
        }

        @Provides
        @Singleton
        public PinotControllerAuthenticationProvider getAuthenticationProvider(PinotControllerBasicAuthenticationConfig config)
        {
            return PinotControllerAuthenticationProvider.create(new PinotBasicAuthenticationProvider(config.getUser(), config.getPassword()));
        }
    }

    private static class PinotBasicBrokerAuthenticationProviderModule
            implements Module
    {
        @Override
        public void configure(Binder binder)
        {
            configBinder(binder).bindConfig(PinotBrokerBasicAuthenticationConfig.class);
        }

        @Provides
        @Singleton
        public PinotBrokerAuthenticationProvider getAuthenticationProvider(PinotBrokerBasicAuthenticationConfig config)
        {
            return PinotBrokerAuthenticationProvider.create(new PinotBasicAuthenticationProvider(config.getUser(), config.getPassword()));
        }
    }
}
